(function(window, document){
  ...
  function insertItem(element, task, id){
    element.insertAdjacentHTML('beforeend', '<li class="todoListItem">' + task + '<button href="#" class="todoListItemDel">&#215;</button></li>');
    // li안에 delete button이 동적으로 생성 되기 때문에 DOM에 추가한 후 클릭 이벤트 바인딩
    bindDeleteButton(id);
  }
  // deleteButton을 바인딩 시켜주는 함수
  function bindDeleteButton(id){
    var delButton = document.querySelectorAll('.todoListItemDel');
  ...
  var input = document.querySelector('.todoListInput');
  console.log('input: ', input);
  var button = document.querySelector('.todoListBtn');
  console.log('button: ', button);
  var todoList = document.querySelector('.todoListItem');
